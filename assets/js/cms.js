(function () {
  ...
  function activeRows(rows) {
    return (rows || []).filter(row => String(row.status || row.STATUS || row.active || row.ACTIVE || 'Active').toLowerCase() !== 'inactive');
  }

  function sortedRows(rows) {
    return activeRows(rows).sort((a, b) => Number(a.sortOrder || a.SORT_ORDER || 999) - Number(b.sortOrder || b.SORT_ORDER || 999));
  }
  ...
  function normalizeData(data) {
    ...
    if (data.tournaments || data.programs || data.schedules || data.resources || data.tournamentLinks || data.TournamentLinks) {
      return {
        ...data,
        tournamentLinks: data.tournamentLinks || data.TournamentLinks || data.tournaments || data.TOURNAMENTS || [],
      };
    }
  ...
  function tournamentNavLinks() {
    ...
    rows.forEach(row => {
      const label = firstValue(row, ['name', 'NAME', 'TOURNAMENT_NAME', 'tournament_name', 'title', 'TITLE']);
      if (!label) return;
      const externalUrl = firstValue(row, [
        'sincsports_url',
        'SINCSports_URL',
        'SINCSports URL',
        'SINCSPORTS_URL',
      ...
      const rowId = firstValue(row, ['tournament_id', 'id', 'TOURNAMENT_ID', 'ID']);
      const webpageUrl = firstValue(row, ['webpage_url', 'WEBPAGE_URL']);
      const fallbackId = tournamentAnchorIds[rowId] || (webpageUrl.includes('#') ? webpageUrl.split('#').pop() : '') || slugify(label);
      links.push({
        label,
        href: isExternalUrl(externalUrl) ? externalUrl : `tournaments.html#${fallbackId}`,
        type: isExternalUrl(externalUrl) ? 'external' : 'pending',
      });
    });
    ...
    qsa('.nav-dropdown').forEach(dropdown => {
      ...
      menu.innerHTML = links.map(link => {
        if (link.type === 'pending') {
          return `<span class="nav-dropdown-link nav-dropdown-link-disabled" aria-disabled="true">${esc(link.label)} <em>Link Coming Soon</em></span>`;
        }
