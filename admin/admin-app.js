...
/* ── CORE STATE ────────────────────────────────────── */
const ADMIN = {
  endpoint: sessionStorage.getItem('rsaAdminEndpoint') || 'https://script.google.com/macros/s/AKfycbxcn3vfdh1UHyOTuEdJGZXT3mc0_aRCDBl3U7doBEJ_eeMkDolW4hsgCgFORQmQD6i4/exec',
...
const TOURNAMENT_LINK_ROWS = {
  'memorial-day-5v5': { id: 'TOURN-001', name: 'Memorial Day 5v5' },
  'summer-elite-5v5': { id: 'TOURN-002', name: 'Summer Elite 5v5' },
  'championship-cup-5v5': { id: 'TOURN-003', name: 'Championship Cup' },
  'elite-7v7-summer': { id: 'TOURN-004', name: 'Elite 7v7 Summer' },
  'labor-day-5v5': { id: 'TOURN-005', name: 'Labor Day 5v5' },
  'winter-elite-5v5': { id: 'TOURN-006', name: 'Winter Elite 5v5' }
};

const PROGRAM_LINK_ROWS = {
  'private-training': { id: 'PROG-001', name: 'Private Training' },
  'elite-pickup': { id: 'PROG-002', name: 'Elite Soccer Pickup' }
};
...
function renderLinkManagers() {
  const tournaments = ADMIN.data.TournamentLinks || [];
  const programs = ADMIN.data.ProgramLinks || [];
  const settings = ADMIN.data.Settings || [];
  const currentRegistration = settings.find(item => (item.SETTING || item.setting) === 'SincSports Link');

  document.querySelectorAll('tr[data-tid]').forEach(row => {
    const key = row.dataset.tid;
    const input = row.querySelector('.tourn-url-input');
    if (!input) return;

    if (key === 'current-registration') {
      input.value = currentRegistration?.VALUE || currentRegistration?.value || '';
      return;
    }

    const config = TOURNAMENT_LINK_ROWS[key];
    const item = tournaments.find(t =>
      t.TOURNAMENT_ID === config?.id ||
      t.tournament_id === key ||
      t.id === key ||
      t.NAME === config?.name ||
      t.name === config?.name
    );
    input.value = item?.REGISTRATION_LINK || item?.sincsports_url || '';
  });

  document.querySelectorAll('tr[data-progid]').forEach(row => {
    const key = row.dataset.progid;
    const input = row.querySelector('.prog-url-input');
    if (!input) return;

    const config = PROGRAM_LINK_ROWS[key];
    const item = programs.find(p =>
      p.PROGRAM_ID === config?.id ||
      p.program_id === key ||
      p.id === key ||
      p.PROGRAM_NAME === config?.name ||
      p.name === config?.name
    );
    input.value = item?.WEBPAGE_URL || item?.url || '';
  });
}
...
document.querySelectorAll('.save-link-btn').forEach(btn => {
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (tid === 'current-registration') {
        await api('upsertRow', {
          sheet: 'SETTINGS',
          data: {
            SETTING: 'SincSports Link',
            VALUE: url,
            DESCRIPTION: 'Master registration link used by homepage CTA',
            LAST_UPDATED: new Date().toISOString(),
            STATUS: 'Active'
          }
        });
      } else {
        const config = TOURNAMENT_LINK_ROWS[tid];
        if (!config) throw new Error(`Unknown tournament: ${tid}`);
        await api('upsertRow', {
          sheet: 'TOURNAMENTS',
          data: {
...
document.querySelectorAll('.save-prog-link-btn').forEach(btn => {
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const config = PROGRAM_LINK_ROWS[progid];
      if (!config) throw new Error(`Unknown program: ${progid}`);
      await api('upsertRow', {
        sheet: 'PROGRAMS',
        data: {
          PROGRAM_ID: config.id,
          PROGRAM_NAME: config.name || name,
          WEBPAGE_URL: url,
          STATUS: 'Active'
        }
      });
...
/* ── ELITE PICKUP INVITE CODES ────────────────────── */
const ELITE_CODES_KEY = 'risingStars_elitePickupCodes';
const ELITE_LINK_KEY = 'risingStars_elitePickupLink';

function getEliteCodes() {
  const stored = localStorage.getItem(ELITE_CODES_KEY);
  return stored ? JSON.parse(stored) : [];
}

function saveEliteCodes(codes) {
  localStorage.setItem(ELITE_CODES_KEY, JSON.stringify(codes));
}

function renderEliteCodes() {
  const tbody = $('#codesTableBody');
  const codes = getEliteCodes();

  if (!codes.length) {
    tbody.innerHTML = '<tr><td colspan="6" class="muted">No invite codes yet. Create one above.</td></tr>';
    return;
  }

  tbody.innerHTML = codes.map((code, idx) => `
    <tr>
      <td style="font-weight:600;font-family:monospace">${code.code}</td>
      <td><span class="pill" style="background:${code.status === 'active' ? 'rgba(34,197,94,.15);color:#22c55e' : 'rgba(239,68,68,.15);color:#ef4444'}">
        ${code.status === 'active' ? 'Active' : 'Inactive'}
      </span></td>
      <td>${code.used || 0}</td>
      <td>${code.limit === 0 ? '∞ (Unlimited)' : code.limit}</td>
      <td style="font-size:12px">${new Date(code.createdAt).toLocaleDateString()}</td>
      <td>
        <button class="btn outline" onclick="toggleEliteCodeStatus('${code.code}')" style="padding:6px 10px;font-size:10px">
          ${code.status === 'active' ? 'Deactivate' : 'Activate'}
        </button>
        <button class="btn outline danger" onclick="deleteEliteCode('${code.code}')" style="padding:6px 10px;font-size:10px;margin-left:4px">Delete</button>
      </td>
    </tr>
  `).join('');
}

function addEliteCode() {
  const codeInput = $('#newCode');
  const statusInput = $('#newCodeStatus');
  const limitInput = $('#newCodeLimit');
  ...
  if (!code) {
    toast('Please enter a code', 'error');
    return;
  }

  if (code.length < 3 || code.length > 20) {
    toast('Code must be 3-20 characters', 'error');
    return;
  }
...
function deleteEliteCode(code) {
  if (!confirm(`Delete code "${code}"?`)) return;

  let codes = getEliteCodes();
...
function toggleEliteCodeStatus(code) {
  const codes = getEliteCodes();
  const codeData = codes.find(c => c.code === code);
  if (codeData) {
    codeData.status = codeData.status === 'active' ? 'inactive' : 'active';
    saveEliteCodes(codes);
    renderEliteCodes();
    toast(`Code ${codeData.status === 'active' ? 'activated' : 'deactivated'} ✓`);
  }
}

function saveAllEliteCodes() {
  const link = $('#elitePickupLinkInput')?.value.trim();
  if (link) {
    localStorage.setItem(ELITE_LINK_KEY, link);
  }
  toast('Elite Pickup settings saved ✓');
}

// Initialize Elite Codes panel
if ($('#addCodeBtn')) {
  $('#addCodeBtn').addEventListener('click', addEliteCode);
  $('#newCode').addEventListener('keypress', (e) => {
    if (e.key === 'Enter') addEliteCode();
  });
  $('#saveCodesBtn').addEventListener('click', saveAllEliteCodes);
  renderEliteCodes();
}
