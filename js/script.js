...
/* ─────────────────────────────────────────────────────
   FORM — validation + Google Sheets + WhatsApp
───────────────────────────────────────────────────── */
function initForm() {
...
function showFormSuccess(form, successEl, payload) {
  ...
  const whatsapp = successEl.querySelector('.success-whatsapp');
  if (whatsapp) whatsapp.href = buildWhatsAppUrl(payload);
  successEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
}

function showFormError(form, message) {
  const fallback = 'We could not save your registration. Please try again or contact us on WhatsApp.';
...
function buildWhatsAppUrl(payload) {
  const msg = encodeURIComponent(
    `Hi! I just registered on risingstarsatlanta.com.\n` +
    `Player: ${payload.playerName} | Age Group: ${payload.ageGroup} | Program: ${payload.program}\n` +
    `Ref: ${payload.regId}`
  );
...
function submitToGoogleAppsScript(payload) {
  if (!CONFIG.GAS_ENDPOINT) {
    return Promise.reject(new Error('Registration is not connected yet. Please contact us on WhatsApp.'));
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    form.hidden = true;
    form.method = 'POST';
    form.action = CONFIG.GAS_ENDPOINT;
